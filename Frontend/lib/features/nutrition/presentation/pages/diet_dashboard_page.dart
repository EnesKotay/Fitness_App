...
class _DietDashboardPageState extends State<DietDashboardPage> {
  ...
  void initState() {
    super.initState();
    _waterGoalGlasses = (StorageHelper.getWaterGoalML() / 250).round().clamp(
      6,
      20,
    );
          ...
          IconButton(
            ...
            onPressed: () {
              try {
                ...
                debugPrint('DietDashboardPage test_mode navigation hatasi: $e');
              }
            },
          ),
      ],
  ...
  Widget _buildMainContent(BuildContext context, DietProvider provider) {
    ...
    if (dateKey != _waterDateKey) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        setState(
          () => _waterGlasses = (ml / 250).round().clamp(0, _waterGoalGlasses),
        );
      });
    }
          ...
          children: [
            ...
            ...MealType.values.map((type) {
              ...
              return Padding(
                  ...
                  .animate()
                  .fadeIn(delay: (180 + index * 50).ms, duration: 400.ms)
  ...
  void _openSearch(BuildContext context, MealType? type) {
    try {
      Navigator.of(
        context,
        rootNavigator: false,
      ).pushNamed('search', arguments: type ?? MealType.snack).then((_) {
        if (mounted) {
          try {
            final provider = Provider.of<DietProvider>(context, listen: false);
