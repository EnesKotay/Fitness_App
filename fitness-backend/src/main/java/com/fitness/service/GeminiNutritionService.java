    ...
    private void applyGoalBasedFiltering(NutritionAiResponse response, String goal) {
        ...
        for (NutritionAiResponse.SuggestedMeal meal : response.meals) {
            ...
            if (passes) {
                ...
                meal.warnings.add("Hedefe tam uymuyor: " + warning.toString());
                // Keep the meal but add warning (alternative approach)
                filteredMeals.add(meal);
            }
        }
        
        // If all filtered out, keep original with warnings
        if (filteredMeals.isEmpty() && !response.meals.isEmpty()) {
            LOG.warn("All meals filtered by goal " + normalizedGoal + ", keeping with warnings");
            response.meals = response.meals.subList(0, Math.min(2, response.meals.size()));
        } else {
