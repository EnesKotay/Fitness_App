 ...
 * Handles API key validation, model selection, timeout, and structured
 * telemetry logging.
 */
@ApplicationScoped
public class GeminiClient {
     ...
     * 
     * @param endpointName  Identifier for the endpoint (e.g., "ai/coach",
     *                      "ai/nutrition")
     ...
     * @return GeminiClientResult containing output text, model used, latency, and
     *         status
     */
    ...
    /**
     * Generate content using Gemini Vision API with image + text prompt.
     * Uses the same fallback/retry/logging infrastructure as generateText.
     *
     * @param endpointName  Identifier for the endpoint
     * @param userId        User ID for logging
     * @param primaryModel  Primary model to try first
     * @param fallbackModel Model to use if primary fails
     ...
     * @param expectJson    Whether to expect JSON response
     * @return GeminiClientResult containing output text
     */
    public GeminiClientResult generateWithImage(
        ...
        for (int i = 0; i < modelCandidates.size(); i++) {
            ...
            try {
                String responseBody = callGeminiWithImage(prompt, imageBytes, mimeType, model, expectJson);
                long latencyMs = System.currentTimeMillis() - startTime;
                String outputText = extractTextFromResponse(responseBody);
                ...
                if (!isLastModel && shouldRetry(statusCode)) {
                    LOG.warnf("Vision: retrying with fallback endpoint=%s userId=%d model=%s status=%d",
                            endpointName, userId, model, statusCode);
                    continue;
                }
    ...
    /**
     * Validate that API key is configured. Throws ServiceUnavailableException if
     * not.
    ...
    private String callGeminiWithImage(String prompt, byte[] imageBytes, String mimeType,
            String model, boolean expectJson)
            throws IOException, InterruptedException {
        String endpoint = "https://generativelanguage.googleapis.com/v1beta/models/"
        ...
        // Build multimodal payload with image + text parts
        var partsArray = objectMapper.createArrayNode();
        partsArray.add(objectMapper.createObjectNode()
        ...
        ObjectNode payload = objectMapper.createObjectNode();
        payload.set("contents", objectMapper.createArrayNode()
                .add(objectMapper.createObjectNode().set("parts", partsArray)));
        ...
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(endpoint))
                .timeout(Duration.ofMillis(Math.max(timeoutMs, 30000))) // Vision needs more time
                .header("Content-Type", "application/json")
                .POST(HttpRequest.BodyPublishers.ofString(objectMapper.writeValueAsString(payload)))
                .build();
